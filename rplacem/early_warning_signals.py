...
def calc_ews(canvas_part_stat, time_window, state_vars = None):
    ...
    if state_vars == None:
        state_vars = [canvas_part_stat.entropy_vst,
                      ...
                      canvas_part_stat.num_pixchanges[0]]
        
    ews = np.zeros((3, len(state_vars),  canvas_part_stat.n_t_bins))
    
    for i in range(len(state_vars)):
        x = state_vars[i]
        ews[0,i,:] = calc_variance(x, time_window)
        ews[1,i,:] = calc_skewness(x, time_window)
        ews[2,i,:] = calc_autocorrelation(x, time_window)
