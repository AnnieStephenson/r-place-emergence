...
def find_all_transitions(keep_idx_comps, stability_vs_time, time_ranges, v, x, y ,z):
    ''' Find and count transitions for compositions of indices keep_idx_comps, using parameters v, x, y ,z '''
    num_comp_with_trans = 0
    num_trans = 0
    num_comp = 0

    for i in keep_idx_comps:
        num_comp += 1
...
for i in range(0, len(cutoffs)):
    ...
    for j in range(0, len(cutoff_stables)):
        print(j)
        vary_cutoffs[j, i] = find_all_transitions(keep_idx_comps, stability_vs_time, time_ranges, cutoffs[i], cutoff_stables[j], num_stab, dist_stab)[1]
...
plt.savefig(os.path.join(os.getcwd(), 'figs', 'Fraction_of_compos_with_transition_vs_cutoff_parameters.png'), bbox_inches='tight')
...
plt.savefig(os.path.join(os.getcwd(), 'figs', 'Fraction_of_compos_with_transition_vs_distance_parameters.png'), bbox_inches='tight')
