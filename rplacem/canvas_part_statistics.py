...
class CanvasPartStatistics(object):
    ...
    def compute_mean_stability(self, cpart, level):
        ...
        res = comp.stability(cpart, np.asarray([0, tmin_all, tmax_all]), level > 1, level > 2, level > 3, True, self.verbose, None, self.t_unit)
    ...
    def compute_stability_vst(self, cpart, level):
        ...
        res = comp.stability(cpart, self.t_ranges, level > 1, level > 2, level > 3, True, self.verbose, None, self.t_unit)
